...
//! # Usage
//! ```bash
//! cargo run -p kvbm-memory --bin validate_numa_placement
//! cargo run -p kvbm-memory --bin validate_numa_placement -- --size 64   # 64 MiB per GPU
//! cargo run -p kvbm-memory --bin validate_numa_placement -- --gpus 0,2  # specific GPUs
//! ```
...
fn main() {
    ...
    println!("NUMA enabled:    {}", kvbm_memory::is_numa_enabled());
    println!();
    ...
    for &gpu_id in &gpus {
        let numa_node = kvbm_memory::numa::get_device_numa_node(gpu_id);
    ...
    for (idx, &gpu_id) in gpus.iter().enumerate() {
        ...
        let storage = match kvbm_memory::PinnedStorage::new_for_device(alloc_size, Some(gpu_id)) {
            Ok(s) => s,
            Err(e) => {
                println!("FAILED: {e}");
                all_ok = false;
